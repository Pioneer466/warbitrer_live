...
export const DEFAULT_GLOBAL_RISK_CONFIG = {
  ...
  mismatchFatalBudgetFractionOfAlignedMargin: 0.5,
} as const;
...
export const globalRiskConfigSchema = z
  .object({
    ...
    mismatchFatalBudgetFractionOfAlignedMargin: z.number().positive().max(1).optional(),
  })
...
export function getMismatchFatalBudgetFraction(config: GlobalRiskConfig) {
  return config.mismatchFatalBudgetFractionOfAlignedMargin ?? 0.5;
}
